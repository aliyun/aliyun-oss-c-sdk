...
void test_select_object_csv_data(CuTest *tc)
{
    ...
    create_test_object_from_file(options, TEST_BUCKET_NAME, object_name, test_local_file, NULL);

    resp_headers = NULL;
    oss_select_object_meta_params_t *meta_params = oss_create_select_object_meta_params(options->pool);
    ...
    long content_length = atol((char*)apr_table_get(resp_headers, OSS_CONTENT_LENGTH));
    CuAssertTrue(tc, content_length == get_file_size(test_local_file));

    const char *sql = "select Year, StateAbbr, CityName, PopulationCount from ossobject where CityName != '' limit 20";
    aos_string_t expression;
    aos_str_set(&expression, sql);
    oss_select_object_params_t *select_params = oss_create_select_object_params(options->pool);
...
void test_select_object_with_output_header(CuTest *tc)
{
    ...
    const char *sql = "select name from ossobject";
    aos_string_t expression;
    aos_str_set(&expression, sql);
    oss_select_object_params_t *select_params = oss_create_select_object_params(options->pool);
    aos_str_set(&select_params->input_param.file_header_info, "USE");
...
void test_select_object_with_keep_columns(CuTest *tc)
{
    ...
    const char *sql = "select _1, _4 from ossobject";
    aos_string_t expression;
    aos_str_set(&expression, sql);
    oss_select_object_params_t *select_params = oss_create_select_object_params(options->pool);
...
void test_select_object_with_gzip_data(CuTest *tc)
{
    ...
    create_test_object_from_file(options, TEST_BUCKET_NAME, object_name, test_local_file_gz, NULL);

    s = oss_head_object(options, &bucket, &object, NULL, &resp_headers);
    CuAssertIntEquals(tc, 200, s->code);

    long content_length = atol((char*)apr_table_get(resp_headers, OSS_CONTENT_LENGTH));
    CuAssertTrue(tc, content_length == get_file_size(test_local_file_gz));

    const char *sql = "select * from ossobject";
    aos_string_t expression;
    aos_str_set(&expression, sql);
    oss_select_object_params_t *select_params = oss_create_select_object_params(options->pool);
    ...
    int64_t select_data_crc = aos_crc64(0, select_data, (size_t)select_data_len);

    cmp_data = get_text_file_data(options->pool, test_local_file);
    cmp_data_len = strlen(select_data);
    int64_t cmp_data_crc = aos_crc64(0, cmp_data, (size_t)cmp_data_len);
...
void test_select_object_big_csv_data(CuTest *tc)
{
    ...
    const char *sql = "select name, company,age from ossobject limit 20000";
    aos_string_t expression;
    aos_str_set(&expression, sql);
    oss_select_object_params_t *select_params = oss_create_select_object_params(options->pool);
    aos_str_set(&select_params->input_param.file_header_info, "USE");
    ...
    CuAssertIntEquals(tc, 20000, col);
    aos_pool_destroy(p);
    printf("%s ok\n", __FUNCTION__);
}
